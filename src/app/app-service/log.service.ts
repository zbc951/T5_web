...
export class LogService {
  ...
  // 交易紀錄類型
  type: reviewType = reviewType.deposit;
  constructor(
    private http: HttpClient
  ) { }

  depositAndWithdrawLog(type): void {
    ...
    //     this.type = reviewType.withdraw;
    //     break;
    //   case reviewType.transfer:
    //     this.type = reviewType.transfer;
    //     break;
    // }
  }
  ...
  walletLogType(formData: { startTime?: string, endTime?: string, type: any }) {

    const params = UtilService.getHttpParams(formData);
    return this.http.get(ApiPath.ApiMemberLogsWallet, { params });
  }

  perDaySumBetLog(formData: { startTime?: string, endTime?: string, platformId: [number], page?: number, logId?: string }) {
  ...
  activityWalletLog(
    formData: {
      receiveSwitch?: string
    }
  ) {
    const params = UtilService.getHttpParams(formData);
    return this.http.get(ApiPath.ApiAactivityAactivityWalletLog, { params });
  }
}
