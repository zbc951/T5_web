...
export class CUI {
  ...
  public static submit(config) {
    ...
    // console.log('Config', Config);
    let { host, port } = Config.Api;

    // console.log(host, port);

    if (host.endsWith('/')) {
      // host = host.substr(-1, 1);
      // console.log('**', host);
  ...
  /**
   * don't use in mobile device
   */
  public static openUrlToListen(title: string, url, callback: (data) => void) {
    const popup = window.open(url, title);
    const checkPopup = () => {
      if (popup.closed) {
        clearInterval(timer);
        window.removeEventListener('message', receiveMessage);
        console.info('window popup closed');
      }
    };
    const receiveMessage = (ev: MessageEvent) => {
      if (location.origin !== ev.origin) {
        console.warn(ev.origin);
        return;
      }
      callback(ev.data);
    };
    window.addEventListener('message', receiveMessage);
    let timer = setInterval(checkPopup, 2 * 1000);
  }
}
